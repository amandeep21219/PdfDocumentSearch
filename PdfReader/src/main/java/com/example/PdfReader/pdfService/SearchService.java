...
import com.example.PdfReader.model.DocumentData;
import com.example.PdfReader.model.Match;
...
public class SearchService {
    ...
    public List<Match> searchInDocument(DocumentData document, String query,Boolean caseSensitive, Integer contextChars) {
        log.info("[SearchService] :: searchInDocument :: Starting search operation for query: '{}' in document: '{}'",
                query, document.getFilename());
    ...
    private String createSnippet(String text, int position, int queryLength, int contextChars) {
        int start = Math.max(0, position - contextChars / 2);//for diving the left and right part of the characters to be shown
